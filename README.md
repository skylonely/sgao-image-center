...
上传页支持在页面按 Ctrl/Command + V 粘贴截图；连续粘贴会追加到待上传列表。粘贴图片按本地时间自动生成 `clipboard-日期-时间` 文件名，重名时追加序号。上传前可查看缩略图、修改文件名、移除单张图片，并用现有目录输入框统一选择保存目录。文件名修改保持图片扩展名，仍经过原有的大小、格式、账号与同名冲突校验；上传成功项从列表移除，失败项保留供重试。在目录或文件名等输入框中粘贴文本不会触发图片入队。
