...
普通删除改为移入回收站；文件管理页可切换回收站、查看受保护的缩略图、恢复到原路径，以及二次确认后彻底删除。不自动清空。批量移入单次最多 50 张，并返回成功项和失败项。回收站也可按记录多选，单次最多 50 张，逐张恢复或彻底删除；彻底删除需要额外勾选确认，失败项保持勾选并逐张显示结果。网络故障后请刷新核对再重试，批量操作不是原子事务。旧版已永久删除的图片不能通过此功能找回。
