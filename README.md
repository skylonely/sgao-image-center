...
配置 `ACCESS_TEAM_DOMAIN`（`https://你的团队.cloudflareaccess.com`）和 `ACCESS_AUD`（该图片应用的 AUD）。当前空配置会拒绝所有管理请求，不能直接发布为空；确认部署前须填入真实值。Worker 验证 Access JWT 的 RS256 签名、issuer、audience、有效期及账号身份，拒绝跨站或缺少 Origin 的写入，不信任请求中自行声明的邮箱。静态资源路由不会传递 `ctx.access`，因此图片 Worker 使用 `jose` 验证签名，而不是沿用 API 的上下文读取方式。参见 [Cloudflare 官方说明](https://developers.cloudflare.com/workers/configuration/cloudflare-access/)。
