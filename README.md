...
图片文件页可导出所选图片，或在完整读取当前筛选结果后导出当前结果。回收站不参与导出。每次最多 20 张、50 MB 原图；超出时需缩小选择或筛选范围。导出在浏览器本机读取公开原图并生成 ZIP，不新增 Worker 接口、不上传文件、不修改 R2，也不会占用 Worker 的下载计算资源。

ZIP 保留原目录，图片存放在 `images/<原路径>`，根目录附带 `backup-manifest.json`。清单记录生成时间、来源、列表元数据、实际下载字节数与读取失败项，方便以后核对或手动恢复。图片本身使用不重复压缩的 ZIP 存储方式，以保留原始字节并避免对 JPEG、PNG、WebP 等已压缩图片浪费浏览器时间。

导出按实际响应大小再次限制；单张读取失败或返回登录 HTML 时会跳过该项并写入清单，其余可读图片仍会导出。所有图片都无法读取、超过 50 MB、取消下载或登录失效时不会下载不完整 ZIP。目录/筛选的“完整结果”仍不是跨请求快照，其他设备的并发变更需刷新后再备份。

上传页的“从备份恢复”只接受图片中心生成的 v1 ZIP：会在本机校验 ZIP 条目、CRC、清单、路径和图片总大小，再展示可勾选的恢复预览。恢复请求逐张保留原路径，后端仍校验 Access 账号、同源请求、图片类型、文件签名和每张 10 MB 上限；不接受压缩/加密 ZIP、未知条目、路径穿越或不匹配的清单。

恢复始终使用条件写入：原路径已有正常图片时跳过，绝不覆盖、重命名或删除线上文件；回收站占位符可安全复用。完成后逐项显示已恢复、同名跳过和失败结果，失败项保留勾选以便重试。导入不恢复导出时的上传时间、ETag、缓存或自定义元数据，也不从回收站自动恢复；它是对 ZIP 中原图字节和原路径的安全重新上传。

## 图片库概览与存储统计

文件管理页的“图片库概览”按需逐页读取正常图片和回收站的元数据，显示两者的文件数、容量、合计占用、一级目录占用、图片格式分布及正常图片中最大的 20 项。统计只读取文件列表，不下载原图、不修改 R2、不清理回收站，也不新增数据库、绑定或后台定时任务。

扫描期间可停止；停止、登录失效、分页异常或达到 10,000 张安全上限时不显示部分总计，以免误认为结果完整。扫描结果是开始到结束期间各分页的读取集合，不是跨分页的一致性快照；其他设备正在上传、移动、删除或恢复时，完成后应重新扫描。每次全库统计会产生正常的 R2 列表读取请求，适合按需查看而非高频刷新。
