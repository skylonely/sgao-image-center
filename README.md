...
上传页支持在页面按 Ctrl/Command + V 粘贴截图；连续粘贴会追加到待上传列表。粘贴图片按本地时间自动生成 `clipboard-日期-时间` 文件名，重名时追加序号。上传前可查看缩略图、修改文件名、移除单张图片，并用现有目录输入框统一选择保存目录。文件名修改保持图片扩展名，仍经过原有的大小、格式、账号与同名冲突校验。在目录或文件名等输入框中粘贴文本不会触发图片入队。

待上传列表逐张显示待上传、上传中、已上传或上传失败状态；失败项展示原因，并可通过“只重试失败项”单独重试，不会重复提交已成功的图片或尚未开始的图片。已上传项保留在当前页面供确认；“清除记录”只清理浏览器列表和预览，不删除线上图片。改名失败项会重新变为待上传，后续使用“开始上传”提交。

## 图片回收站

普通删除改为移入回收站；文件管理页可切换回收站、查看受保护的缩略图、恢复到原路径，以及二次确认后彻底删除。不自动清空。批量移入单次最多 50 张，并返回成功项和失败项。回收站也可按记录多选，单次最多 50 张，逐张恢复或彻底删除；彻底删除需要额外勾选确认，确认后立即关闭弹框并在列表上方显示进度与结果，失败项保持勾选。网络故障后请刷新核对再重试，批量操作不是原子事务。旧版已永久删除的图片不能通过此功能找回。
...
文件管理页的每张正常图片都可直接“替换”，地址和路径保持不变。服务端在条件写入新图前，先把当前图的原始字节、HTTP 元数据和自定义元数据保存为私有历史版本；如果任一步失败，当前线上图片不会被覆盖。替换和恢复都携带 ETag 与 R2 版本号，其他设备刚修改过同一张图时会拒绝操作并要求刷新，不会静默覆盖。

“历史”可列出、预览和恢复旧图；预览接口受 Cloudflare Access 保护，历史对象也不会出现在普通列表、目录扫描或公开图片路由中。恢复旧图时，恢复前的当前图会再保存为历史版本。每个路径最多保留最近 5 个旧版本，超过时自动清理最早版本；历史版本不是备份的替代品，重要图片仍应使用 ZIP 导出保存独立副本。

## 图片标签与收藏

文件管理页可为每张正常图片设置最多 12 个标签，也可标记为收藏；标签采用图片的 R2 自定义元数据保存，不引入额外数据库或新的绑定。标签支持按项筛选，收藏可单独筛选；筛选时会继续读取完整的文件列表，避免只对当前页得出不完整结果。

标签或收藏更新使用图片的 ETag 和 R2 版本号校验，其他设备已修改同一张图片时会拒绝写入而不会覆盖。标签会随图片移动、进入回收站、恢复和版本替换一同保留；恢复到历史版本时也会恢复该历史版本当时的标签状态。

勾选 1～50 张正常图片后可批量添加标签、移除标签，并统一设为收藏或取消收藏；三种操作可组合，未指定的内容保持不变。批量请求逐张条件写入并返回成功项与失败项，不是整体事务：成功项取消选择，版本冲突、标签数量超限或其他失败项继续保留选择，方便刷新后重试。网络中断导致结果不确定时不会自动重发，需先刷新检查，避免重复操作覆盖新版本。
