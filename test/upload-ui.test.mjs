...
test('paste in text fields is ignored; upload uses the selected folder and clears successful previews', async () => {
	const h = harness();
	const screenshot = new File(['image'], 'screen.png', { type: 'image/png' });
...
test('failed uploads stay queued for retry', async () => {
	...
	await h.nodes.get('#uploadButton').handlers.get('click')();
	assert.equal(h.run('selectedFiles.length'), 1);
	assert.equal(h.nodes.get('#fileList').children.length, 1);
